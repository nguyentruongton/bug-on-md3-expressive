    ...
    <div className="my-8 rounded-m3-xl border border-m3-outline-variant bg-m3-surface-container-low p-6 shadow-sm">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex-1">
          <label htmlFor="md3-source-color-picker" className="block text-sm font-medium text-m3-on-surface mb-3">
            Source Color (Hex)
          </label>
        ...
        <div className="flex flex-col">
          <span id="theme-mode-label" className="block text-sm font-medium text-m3-on-surface mb-3">
            Theme Mode
          </span>
          <div role="group" aria-labelledby="theme-mode-label" className="flex bg-m3-surface-container-highest rounded-m3-full p-1 border border-m3-outline-variant">
            <button
              type="button"
      ...
      {/* Preset Colors */}
      <div className="mt-8">
        <span id="presets-label" className="block text-sm font-medium text-m3-on-surface-variant mb-3">
          Quick Presets
        </span>
        <div role="group" aria-labelledby="presets-label" className="flex flex-wrap gap-3">
          {presets.map((p) => (
            <button
              key={p.color}
              type="button"
      ...
      <p className="mt-6 text-xs text-m3-on-surface-variant bg-m3-surface-variant/30 p-3 rounded-m3-md border border-m3-outline-variant/30 leading-normal italic">
        Thử đổi màu hoặc theme để thấy toàn bộ trang tài liệu này cập nhật tokens theo thuật toán Material Design 3.
      </p>
    </div>
  );
}
